...
def train_siamese(data_dict, subset = 0, epochs = 100, save_network_path = "trained_model/trained_network.pt", save = True):
    ...
    for epoch in range(epochs):
        ...
        for i in tqdm(range(all_dict_keys_len - subset)):
            ...
            # todo - refactor below (should have call these three times)
            ...
            anchor_output = network.forward(anchor_input)
            positive_output = network.forward(positive_input)
            negative_output = network.forward(negative_input)
